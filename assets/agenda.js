/* =====================================================================
   Seu-Barbex — motor de agendamento
...
(function () {
  ...
  var CHAVE = 'sb_agendamentos_demo';
  ...
  function demoBloqueios() {
    try { return JSON.parse(localStorage.getItem('sb_bloqueios_demo') || '[]'); }
    catch (e) { return []; }
  }
  ...
  function demoSemeia() {
    try { if (localStorage.getItem('sb_demo_semeado_v2')) return; } catch (e) { return; }
    ...
    demoGrava(lista);
    try { localStorage.setItem('sb_demo_semeado_v2', '1'); } catch (e) {}
  }
  ...
  try {
    var salvo = JSON.parse(localStorage.getItem('sb_cliente') || 'null');
  ...
  form.addEventListener('submit', function (ev) {
    ...
    API.criar({
      ...
      try { localStorage.setItem('sb_cliente', JSON.stringify({ nome: nome, tel: campoTel.value })); } catch (e) {}
  ...
  function mostraSucesso(a, nome) {
    ...
    var texto =
      'Olá, Seu-Barbex! Marquei pelo site.\n\n' +
    ...
    $('#ok-ics', raiz).download = 'seu-barbex-' + a.codigo + '.ics';
  ...
  function fazIcs(a) {
    ...
    var linhas = [
      'BEGIN:VCALENDAR', 'VERSION:2.0', 'PRODID:-//Seu-Barbex//PT-BR',
      'BEGIN:VEVENT',
      'UID:' + a.codigo + '@seu-barbex',
      'DTSTART;TZID=America/Bahia:' + carimbo(a.dia, a.inicio),
      'DTEND;TZID=America/Bahia:'   + carimbo(a.dia, a.fim),
      'SUMMARY:' + a.servico + ' com ' + a.barbeiro + ' — Seu-Barbex',
